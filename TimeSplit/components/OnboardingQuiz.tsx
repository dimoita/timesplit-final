...
export const OnboardingQuiz: React.FC<OnboardingQuizProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  // --- LISTA FINAL DE IMAGENS (Sua curadoria) ---
  const stepImages = [
    // 0: Identidade (Criança confiante - Seu Link)
    ...
    // 1: Dor (Criança frustrada - Seu Link)
    ...
    // 2: Sonho (Criança comemorando - Seu Link)
    ...
    // 3: Compromisso (High Five - Seu Link)
    ...
    // 5: Sucesso (Pai e filho felizes - SEU NOVO LINK A - Horizontal)
    "https://images.unsplash.com/photo-1666593409306-216c0245c4ab?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"  
  ];
    ...
    <div className="fixed inset-0 z-[100] bg-white font-nunito animate-in fade-in duration-300 flex flex-col md:flex-row h-[100dvh] w-screen overflow-hidden">
      ...
      <div className="relative w-full h-[25%] md:h-full md:w-1/2 bg-slate-900 overflow-hidden shrink-0">
        ...
        {step === 4 ? (
            ...
            // IMAGENS (Com object-center para garantir o foco)
            <>
                <div className="absolute inset-0 bg-black/10 z-10"></div>
                <img 
                    src={stepImages[step]} 
                    alt="Quiz Context" 
                    className="w-full h-full object-cover object-center transition-all duration-700 transform scale-105 opacity-95 bg-slate-200"
                />
            </>
        )}
