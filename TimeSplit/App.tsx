...
export default function App() {
  ...
  // NOME TEMPORÁRIO DO QUIZ (Para o Checkout)
  ...
  const persistData = useCallback(async (newProfiles: Profile[], forceCloud: boolean = false) => {
      ...
      syncTimeoutRef.current = setTimeout(async () => {
          try {
              ...
              setSyncStatus('ERROR');
              addToast("A internet oscilou. Seus dados estão salvos no dispositivo.", "ERROR");
          }
      }, 1000); 
  }, [user]);
      ...
      if (profile) {
          ...
          if (profile.progress.lastPlayedDate !== today) {
              updateCurrentProfile(p => ({
                  ...p,
                  progress: { ...p.progress, dailyLevelCount: 0, lastPlayedDate: today }
              }));
          }
  ...
  const handleStartSession = (levelId: number) => {
      ...
      if (p) {
          ...
          const isNextLevel = levelId > p.progress.unlockedLevel - 1; 
          if (isNextLevel && playedToday >= DAILY_LEVEL_LIMIT) {
              addToast("Protocolo Diário Completo! Volte amanhã.", "WARNING");
              return; 
          }
      }
  ...
  const handleGameComplete = async (stats: GameStats) => {
      ...
      updateCurrentProfile(p => {
          ...
          const newChronicle: Chronicle = {
              id: Date.now().toString(), text: chronicleText, level: activeLevelId, timestamp: new Date().toLocaleDateString()
          };
          let dailyCount = p.progress.dailyLevelCount || 0;
          if (isWin && activeLevelId === p.progress.unlockedLevel) { dailyCount += 1; }
          return {
              ...p, progress: { ...p.progress, coins: newCoins, totalStars: newStars, unlockedLevel: nextLevel, chronicles: [newChronicle, ...p.progress.chronicles], unlockedArtifacts: updatedArtifacts, dailyLevelCount: dailyCount }
          };
      }, true);
      setView('DASHBOARD');
  ...
  const handleRedeemCode = (code: string): boolean => {
      ...
      if (code.startsWith('AGENT-') && code.length > 7 && code !== p.progress.referralCode) {
          updateCurrentProfile(prof => ({
              ...prof, progress: { ...prof.progress, coins: prof.progress.coins + 500, redeemedCode: true }
          }), true);
          return true;
      }
      return false;
  ...
  const handleApproveItem = (itemId: string) => {
      updateCurrentProfile(p => ({
          ...p, progress: { ...p.progress, realWorldInventory: (p.progress.realWorldInventory || []).map(item => item.id === itemId ? { ...item, status: 'USED' } : item) }
      }), true);
  ...
  const handleRefundItem = (itemId: string) => {
      updateCurrentProfile(p => {
          ...
          return {
              ...p, progress: { ...p.progress, coins: p.progress.coins + refundAmount, realWorldInventory: (p.progress.realWorldInventory || []).filter(i => i.id !== itemId) }
          };
      }, true);
  ...
  if (view === 'LOGIN') {
      return (
          <LoginPage 
              onLoginSuccess={() => { setView('PROFILES'); }} 
  ...
  if (view === 'LANDING') {
      return (
          <>
            ...
            {/* O QUIZ CHAMA A FUNÇÃO DE CHECKOUT */}
            <OnboardingQuiz 
                isOpen={showQuiz} 
    ...
    <>
        ...
        {view === 'DASHBOARD' && (
            <Dashboard 
                ...
                onOpenTitan={() => { setView('TITAN'); }}
                onOpenBase={() => setView('HQ')}
                onOpenPrintables={() => setView('PRINT')}
                onBuyConsumable={(id, cost) => updateCurrentProfile(p => ({ ...p, progress: { ...p.progress, coins: p.progress.coins - cost, consumables: { ...p.progress.consumables, [id]: (p.progress.consumables[id] || 0) + 1 } } }))}
                onClaimBounty={(id) => updateCurrentProfile(p => ({ ...p, progress: { ...p.progress, coins: p.progress.coins + 50, bounties: p.progress.bounties.filter(b => b.id !== id) } }), true)}
                onTriggerCheckout={() => { setCheckoutBump(null); setShowCheckout(true); }}
                activeEvent={activeEvent}
                onEventEnd={() => setActiveEvent(null)}
                onAddCoins={(amount) => updateCurrentProfile(p => ({ ...p, progress: { ...p.progress, coins: p.progress.coins + amount } }), true)}
                dailyLevelCount={activeProfile.progress.dailyLevelCount || 0} 
            />
        )}
